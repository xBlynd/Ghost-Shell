...
def main():
    ...
    try:
        ...
        if headless:
            ...
            # Ghost Phoenix Entry Screen (TUI)
            try:
                from src.tui.entry_screen import GhostEntryApp
                entry_app = GhostEntryApp(kernel)
                result = entry_app.run()
                
                if result == "shutdown":
                    print("[Ghost] Shutdown requested.")
                    sys.exit(0)
                elif result == "exit":
                    print("[Ghost] Termination requested.")
                    sys.exit(0)
                elif result == "guest_success":
                    print("[Ghost] Guest access granted.")
                    if kernel.engine_available("security"):
                        kernel.engines["security"].current_role = "GUEST"
                        kernel.engines["security"].authenticated = True
                elif result == "login_success":
                    print("[Ghost] Access granted. Welcome back.")
                    if kernel.engine_available("security"):
                        # Ensure we are GOD since they provided the masterkey
                        kernel.engines["security"].current_role = "GOD"
                        kernel.engines["security"].authenticated = True
                ...
                if debug:
                    import traceback
                    traceback.print_exc()
                print(f"\n[!] Entry Screen failed ({e}). Falling back to standard shell...")
